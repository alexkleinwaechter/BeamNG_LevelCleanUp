    ...
    public static byte[] ProcessMaterialLayers(List<MaterialDefinition> materials, int size)
    {
        ...
        try
        {
            for (var i = 0; i < materials.Count; i++)
            {
                ...
                if (string.IsNullOrWhiteSpace(mat.LayerImagePath)) continue;
            ...
            if (useParallel)
                ...
                Parallel.For(0, size, y => { ProcessRow(y, size, layerData, materialIndices); });
            else
        ...
        finally
        {
            ...
            foreach (var (_, image) in loadedImages) image.Dispose();
        }
