...
public class SkeletonizationRoadExtractor
{
    public List<List<Vector2>> ExtractCenterlinePaths(byte[,] roadMask, RoadSmoothingParameters? parameters = null)
    {
        ...
        var totalPixels = (long)height * width;

        perfLog?.Timing($"Skeleton extraction started: {width}x{height} = {totalPixels:N0} pixels");
        ...
        for (var x = 0; x < width; x++)
            if (binaryMask[y, x])
                roadPixels++;
        perfLog?.Timing(
            $"ConvertToBinaryMask: {sw.ElapsedMilliseconds}ms, {roadPixels:N0} road pixels ({100.0 * roadPixels / totalPixels:F2}% coverage)");
        ...
        if (dilationRadius > 0)
        {
            ...
            perfLog?.Timing($"DilateMask (radius={dilationRadius}): {sw.ElapsedMilliseconds}ms");
        ...
        sw.Restart();
        perfLog?.Timing("Starting Zhang-Suen thinning (this may take a while for large terrains)...");
        var skeleton = ApplyZhangSuenThinning(binaryMask);
        var thinningTime = sw.Elapsed;
        ...
        for (var x = 0; x < width; x++)
            if (skeleton[y, x])
                skeletonPixels++;
        perfLog?.Timing(
            $"ApplyZhangSuenThinning: {thinningTime.TotalSeconds:F2}s, {skeletonPixels:N0} skeleton pixels");
        ...
        for (var x = 0; x < width; x++)
            if (skeleton[y, x])
                prunedPixels++;
        perfLog?.Timing(
            $"PruneShortSpurs (length={pruneLength}): {pruneTime.TotalSeconds:F2}s, removed {skeletonPixels - prunedPixels} spur pixels");
        TerrainLogger.Info(
        ...
        perfLog?.Timing(
            $"DetectKeypoints: {sw.ElapsedMilliseconds}ms, {endpoints.Count} endpoints, {junctions.Count} junctions");
        ...
        perfLog?.Timing(
            $"ExtractPathsFromEndpointsAndJunctions: {sw.ElapsedMilliseconds}ms, {rawPaths.Count} raw paths");
        TerrainLogger.Info($"Extracted {rawPaths.Count} raw paths");

        // Join close paths (use parameter or default to 40 pixels)
        sw.Restart();
        var joinThreshold = parameters?.SplineParameters?.BridgeEndpointMaxDistancePixels ?? 40.0f;
        rawPaths = JoinClosePaths(rawPaths, joinThreshold);
        perfLog?.Timing(
            $"JoinClosePaths (threshold={joinThreshold}): {sw.ElapsedMilliseconds}ms, {rawPaths.Count} paths");
        ...
        {
            var densified = DensifyPath(path, parameters?.SplineParameters?.DensifyMaxSpacingPixels ?? 1.0f);
            if (densified.Count > 1) orderedPaths.Add(densified);
        }

        perfLog?.Timing($"DensifyPaths: {sw.ElapsedMilliseconds}ms, {orderedPaths.Count} final paths");

        if (parameters?.SplineParameters?.ExportSkeletonDebugImage == true)
            try
            {
        ...
        totalSw.Stop();
        perfLog?.Timing($"=== Skeleton extraction TOTAL: {totalSw.Elapsed.TotalSeconds:F2}s ===");
        perfLog?.LogMemoryUsage("After skeleton extraction");
    ...
    private bool[,] ApplyZhangSuenThinning(bool[,] mask)
    {
        ...
        do
        {
            ...
            // Log progress every 5 seconds or every 10 iterations
            var elapsed = iterationSw.Elapsed;
            if (elapsed - lastLogTime > TimeSpan.FromSeconds(5) || iteration % 10 == 0)
            {
                perfLog?.Timing($"  Thinning iteration {iteration}: elapsed {elapsed.TotalSeconds:F1}s");
                lastLogTime = elapsed;
            }
        } while (changed && iteration < 100);

        perfLog?.Timing($"  Thinning complete: {iteration} iterations in {iterationSw.Elapsed.TotalSeconds:F2}s");
