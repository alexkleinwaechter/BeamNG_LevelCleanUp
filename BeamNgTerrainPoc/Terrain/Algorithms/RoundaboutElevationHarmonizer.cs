...
public class RoundaboutElevationHarmonizer
{
        ...
        float globalBlendDistance = 30.0f)
    {
        ...
        foreach (var roundaboutInfo in roundaboutJunctionInfos)
        {
            ...
            TerrainLogger.Detail($"  Roundabout {ringSplineId}: {connectingBlended} connecting road cross-sections blended");
            ...
            foreach (var junction in roundaboutInfo.Junctions)
            {
                if (junction.ParentJunction != null)
                {
                    ...
                    junction.ParentJunction.ExclusionReason = "Roundabout junction - handled by RoundaboutElevationHarmonizer";
                }
            }
        }
        ...
        TerrainLogger.Info($"  RESULT: {result.ConnectingRoadCrossSectionsBlended} connecting road cross-sections blended");
        ...
        UnifiedRoadNetwork network)
    {
        ...
        if (roundaboutInfo.Junctions.Count == 0)
        {
            return averageTerrainElevation;
        }
