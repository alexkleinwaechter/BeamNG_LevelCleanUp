...
public class GroundCoverCopyScanner
{
    ...
    public List<string> GroundCoverJsonLines { get; } = new();
    ...
    {
        if (!itemsLevelFile.Exists) return;
        ...
        foreach (var line in File.ReadAllLines(itemsLevelFile.FullName))
            try
            {
                using var jsonObject = JsonUtils.GetValidJsonDocumentFromString(line, itemsLevelFile.FullName);
