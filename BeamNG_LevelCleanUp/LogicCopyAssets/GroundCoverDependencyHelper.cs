...
public class GroundCoverDependencyHelper
{
        ...
        string targetNamePath)
    {
        ...
        _targetLevelName =
            Path.GetFileName(_targetNamePath.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar));
    ...
    public void LoadMaterialsJsonCopy(List<MaterialJson> materialsJsonCopy)
    {
        ...
        foreach (var group in
                 terrainMaterials.GroupBy(m => m.Name))
        ...
        if (groundCoverNode["Types"] is JsonArray types)
            foreach (var type in types)
                if (type != null && type["shapeFilename"] != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(daeFilePath)) CopyGroundCoverDaeFile(daeFilePath);
                }
