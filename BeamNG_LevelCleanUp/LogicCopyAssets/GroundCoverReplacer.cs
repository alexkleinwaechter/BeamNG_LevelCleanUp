...
namespace BeamNG_LevelCleanUp.LogicCopyAssets
{
    ...
    public class GroundCoverReplacer
    {
       ...
       : this(dependencyHelper, namePath)
        ...
        {
            if (string.IsNullOrEmpty(targetMaterialKey) || sourceMaterials == null || !sourceMaterials.Any())
            {
                return;
            }
        ...
        public void WriteAllGroundCoverReplacements()
        {
           ...
           $"Processing {_groundCoversToReplace.Count} groundcover replacement(s)...");
            ...
            var targetGcFile = new FileInfo(Path.Join(_namePath, "main", "MissionGroup", "Level_object", "vegetation", "items.level.json"));
            ...
            {
                PubSubChannel.SendMessage(PubSubMessageType.Warning, "No replaced material internal names collected. Skipping modification phase.");
            }
            else
            {
                PubSubChannel.SendMessage(PubSubMessageType.Info,
            ...
            $"Found {sourceGroundCovers.Count} source groundcover(s) for material '{sourceInternalName}'. Copying to target with level suffix.");
                    ...
                    foreach (var sourceGroundCover in sourceGroundCovers)
                    {
                        ...
                        if (string.IsNullOrEmpty(sourceGroundCoverName))
                        {
                            continue;
                        }
            ...
            {
                WriteGroundCoverChangesToFile(targetGcFile, existingGroundCovers, newGroundCovers, modifiedGroundCovers, deletedGroundCoverNames);
            }
            else
            {
                PubSubChannel.SendMessage(PubSubMessageType.Warning,
        ...
        private bool GroundCoverReferencesLayer(JsonNode groundCover, string materialName)
        {
            ...
            foreach (var type in types)
            {
                ...
                if (!string.IsNullOrEmpty(layer) && string.Equals(layer, materialName, StringComparison.OrdinalIgnoreCase))
                {
                    return true;
                }
            }
        ...
        /// </summary>
        private (bool modified, bool shouldDelete) RemoveReplacedLayers(JsonNode groundCover, HashSet<string> replacedMaterialNames)
        {
            if (groundCover["Types"] is not JsonArray types)
            {
                return (false, false);
            ...
            if (replacedMaterialNames == null || replacedMaterialNames.Count ==0)
            {
                return (false, false);
            }
                    ...
                    try
                    {
                        ...
                        if (gcNode != null)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    existingGroundCovers[persistentId] = gcNode;
                                }
                            }
                        }
                    }
         ...
         List<string> deletedGroundCoverNames)
        {
            try
            {
                ...
                foreach (var modifiedGC in modifiedGroundCovers)
                {
                    ...
                    if (!string.IsNullOrEmpty(name))
                    {
                        existingGroundCovers[name] = modifiedGC;
                    }
                }

                // Apply deletions
                foreach (var deletedName in deletedGroundCoverNames)
                {
                    existingGroundCovers.Remove(deletedName);
                }
