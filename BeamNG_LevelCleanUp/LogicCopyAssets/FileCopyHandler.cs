    ...
    public class FileCopyHandler
    {
        ...
        private void TryExtractFromZip(string sourceFile, string targetFile)
        {
            try
            {
                ...
                if (!new FileInfo(beamZip).Exists || thisLevelName.Equals(_levelNameCopyFrom, StringComparison.InvariantCultureIgnoreCase))
                {
            ...
            {
                // Silently ignore if we can't extract from zip, maintaining original behavior
                throw;
            }
        }
